...
int loadCharacter(int* s, int i)
{
  ...
  return rightShift(leftShift(*(s + a), ((SIZEOFINT - 1) - (i % SIZEOFINT)) * 8),
      (SIZEOFINT - 1) * 8);
}
...
int* storeCharacter(int* s, int i, int c)
{
  ...
  *(s + a) = (*(s + a) - leftShift(loadCharacter(s, i), (i % SIZEOFINT) * 8)) +
      leftShift(c, (i % SIZEOFINT) * 8);
  ...
  if( value >= 0 )
  {
    ...
    else if( value < twoToThePowerOf(28) )
    {
      ...
      emitIFormat(OP_ADDIU, currentTemporary(), currentTemporary(),
          rightShift(leftShift(value, 18), 18));
    }
    else
    {
      ...
      emitIFormat(OP_ADDIU, currentTemporary(), currentTemporary(),
          rightShift(leftShift(value, 15), 18));

      emitLeftShiftBy(3);

      // and finally add the remaining 3 lsbs
      emitIFormat(OP_ADDIU, currentTemporary(), currentTemporary(),
          rightShift(leftShift(value, 29), 29));
    }
  }
  else
  {
      ...
      if( symbol == SYM_IDENTIFIER )
      {
        ...
        else
        {
          ...
          if( entry == (int*)0 )
          {
            ...
            createSymbolTableEntry(GLOBAL_TABLE, variableOrProcedureName, currentLineNumber,
                VARIABLE, type, initialValue, -allocatedMemory);
          }
          else
          {
...
void selfie_compile()
{
  ...
  while( link )
  {
    ...
    else
    {
      ...
      print( (int*)"(" );
      printFixedPointPercentage(numberOfReadCharacters,
          numberOfReadCharacters - numberOfIgnoredCharacters);
